                        ...
                        <tr class="form-body">
                            <div class="row">
                                ...
                                <div class="col-md-5">
                                    ...
                                    <b>Extra items:</b> {!! count($location->extraItems()) !!}<br>
                                </div>
                            </div>
                            <hr>

                            @foreach ($location->extraItems() as $extra)
                                <h3>{{ $extra->equipment->name }} -
                                    <small>Extra:{{ $extra->extra }}</small>
                                </h3>
                                    <table class="table table-striped table-bordered table-hover order-column" id="table_list">
                                        ...
                                        @foreach ($extra->equipment->lost as $lost)
                                            <?php $max_trans = min($extra->extra,  $lost->qty); ?>
                                            @if ($lost->equipment_id == $extra->equipment_id)
                                                <tr>
                                                    <td>{{ $lost->location->name }}</td>
                                                    <td>{{ $lost->qty }}</td>
                                                    <td>
                                                        <select id="qty-{{ $lost->id }}-{{ $extra->extra }}" name="qty-{{ $lost->id }}-{{ $extra->extra }}" class="form-control bs-select" width="100%">
