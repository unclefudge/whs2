...
trait UserRolesPermissions {
     ...
     */
    /*
    public function allowedTo($action, $permission, $record = '')
    {
        // Todoo
        if ($permission == 'todo') {
            if ($action == 'add')
        ...
        if ($action == 'add')
            return $this->can("add.$permission|add.p.$permission");
        else {
            // Users ***
            if ($permission == 'user') {
                if ($record->id == $this->id && ($action == 'view' || $action == 'edit')) // User can always view/edit own profile
                    return true;

                return ($permission == 'user' && $this->can("$action.user") && in_array($record->id, $this->company->users()->pluck('id')->toArray()));
            }

            // Companies ***
            if ($permission == 'company') {
                if ($action == 'del' && $record->id == $this->company_id) // User can't delete own company
                    return false;

                return ($this->can("$action.company") && in_array($record->id, $this->company->companies()->pluck('id')->toArray()));
            }

            // Company Documents **
            if ($permission == 'company.doc') {
                if ($this->can("$action.$permission|$action.p.$permission") && $record->company_id == $this->company_id) // User belong to same company record
                    return true;
                if ($this->can("$action.company") && $record->for_company_id == $this->company_id && ($record->status == '2' || $record->status == '3')) // User had edit access for company doc record
                    return true;
            ...
            // Sites ***
            if ($permission == 'site') {
                if ($this->can("$action.$permission")) {
                    if ($this->is('con.manager|area.supervisor|supervisor') && in_array($record->id, $this->siteListArray()))  // User manages site record
                        return true;
                    else if (!$this->is('con.manager|area.supervisor|supervisor') && in_array($record->id, $this->company->sites()->pluck('id')->toArray())) // User manages site record
                        return true;
                }
            }

            // Site Doc Manager ***
            if ($permission == 'site.doc.manager' && $action == 'view')
                return ($this->can("add.site.doc|edit.site.doc|del.site.doc|add.safety.doc|edit.safety.doc|del.safety.doc") && $this->company->subscription);

            // Site Doc Manager ***
            if ($permission == 'upload.manager' && $action == 'view')
                return ($this->can("add.site.doc|edit.site.doc|del.site.doc|add.safety.doc|edit.safety.doc|del.safety.doc") && $this->company->subscription);

            // Site Export ***
            if ($permission == 'site.export') {
                if ($this->can("$action.$permission"))
                    return true;
            }

            // Site Doc Manager + Safety Reports + Compliance ***
            if ($permission == 'site.doc' || $permission == 'safety.doc' || $permission == 'compliance') {
                if ($this->can("$action.$permission")) {
                    if ($this->is('con.manager|area.supervisor|supervisor') && in_array($record->site->id, $this->siteListArray())) // User manages site record
                        return true;
                    else if (!$this->is('con.manager|area.supervisor|supervisor') && in_array($record->site->id, $this->company->sites()->pluck('id')->toArray())) // User manages site record
                        return true;
                }
            }
            ...
            if ($permission == 'attendance') {
                ...
                if ($this->can("$action.p.$permission")) {
                    if ($this->is('leading.hand|contracter.admin|leading.hand') && in_array($record->id, $this->company->sitesPlannedFor()->pluck('id')->toArray())) // User planned on site record
                        return true;
                }
            ...
            // Accident Reports ***
            if ($permission == 'site.accident') {
                if ($record->created_by == $this->id && $action == 'view') // User created record
                    return true;
                if ($this->can("$action.$permission|$action.p.$permission")) {
                    if ($this->is('con.manager|area.supervisor|supervisor') && in_array($record->site->id, $this->siteListArray())) { // User manages site record
                        return true;
                    } elseif (!$this->is('con.manager|area.supervisor|supervisor') && in_array($record->created_by, $this->company->users()->pluck('id')->toArray())) { // User belongs to same company
                        return true;
                    }
                }
            ...
            // Hazard Reports ***
            if ($permission == 'site.hazard') {
                if ($record->created_by == $this->id && ($action == 'view' || $action == 'edit')) // User can always view/edit own record
                    return true;
                if ($this->can("$action.$permission|$action.p.$permission")) {
                    if ($this->is('con.manager|area.supervisor|supervisor') && in_array($record->site_id, $this->siteListArray())) { // User manages site record
                        return true;
                    } elseif (!$this->is('con.manager|area.supervisor|supervisor') && in_array($record->created_by, $this->company->users()->pluck('id')->toArray())) { // User belongs to same company
                        return true;
                    }
            ...
            // Toolbox ***
            if ($permission == 'toolbox') {
                if ($action == 'view' && $record->isAssignedToUser($this))
                    return true;
                if ($this->can("$action.$permission|$action.p.$permission") &&
                    (in_array($record->for_company_id, $this->company->companies()->pluck('id')->toArray()) || in_array($record->owned_by->id, $this->company->companies()->pluck('id')->toArray()))
                ) // User manages company record
                    return true;
                if ($action == 'view' && $record->master && $record->company_id == $this->company->reportsToCompany()->id) // User can view parents templates
                    return true;
            ...
            // SDS ***
            if ($permission == 'sds') {
                if ($action == 'view' && $this->can("view.sds|view.p.sds"))
                    return true;
                if ($this->can("$action.$permission|$action.p.$permission") && ($record->company_id == $this->company_id)) // User belong to same company record
                    return true;
            ...
            // Site QA ***
            if ($permission == 'site.qa') {
                if ($this->can("$action.$permission|$action.p.$permission")) {
                    if ($this->is('con.manager|area.supervisor|supervisor') && in_array($record->site->id, $this->siteListArray())) { // User manages site record
                        return true;
                    } elseif (!$this->is('con.manager|area.supervisor|supervisor') && in_array($record->owned_by->id, $this->company->users()->pluck('id')->toArray())) { // User belongs to same company
                        return true;
                    }
                }
            ...
            // Weekly + Site + Trade Planners ***
            if ($permission == 'weekly.planner' || $permission == 'site.planner' || $permission == 'trade.planner') {
                if ($this->can("$action.$permission")) {
                    if ($this->is('con.manager|area.supervisor|supervisor') && in_array($record->id, $this->siteListArray())) // User manages site record
                        return true;
                    else if (!$this->is('con.manager|area.supervisor|supervisor') && in_array($record->id, $this->company->sites()->pluck('id')->toArray())) // User manages site record
            ...
            // Support Tickets **
            if ($permission == 'support.ticket') {
                if ($record->created_by == $this->id && ($action == 'view' || $action == 'edit')) // User can always view/edit own record
                    return true;
                if ($this->security && in_array($record->created_by, $this->company->users()->pluck('id')->toArray())) // User planned on site record
                    return true;
            ...
            // Safetytip + Notify **
            if ($permission == 'safetytip' || $permission == 'notify') {
                if ($this->can("$action.$permission"))
                    return true;
            }
